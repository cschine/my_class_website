...
gss_cat |>
  ...
  summarize(count=n()) |>
ggplot() +
  geom_col(aes(x=partyid, y=count))
...
# 5.
What are the levels of the `relig` variable? Create a table which counts how many people belong to each religion. Be sure to print out your table in a pretty way using the `kableExtra` package. 
...
gss_cat |>
  mutate(top_relig = relig |> fct_lump_n(5)) |>
  count(top_relig) |>
  kbl() |>
  kable_styling()
...
gss_cat |>
  ggplot() +
  geom_boxplot(aes(x=marital, y=tvhours)) +
  theme_tufte()
...
gss_cat |>
  ...
  summarize(mar_count=n()) |>
  ggplot() +
  geom_line(aes(x=year, y=mar_count, color=marital))

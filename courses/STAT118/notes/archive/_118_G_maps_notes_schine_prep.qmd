...
# add color palette using RColor Brewer and scale_fill_distiller(palette="")
...
nc <- nc |> mutate(nonwhite_births = NWBIR74 / BIR74)
nc <- nc |> mutate(SIDS_deaths_per_birth = SID74 / BIR74)
# we are dividing by total births to account for difference in population denisty

nc|>
  ggplot()+
  geom_sf(aes(fill=nonwhite_births))+
  scale_fill_viridis(option="cividis", direction = -1)+
  labs(fill="Proportion of Nonwhite Births")+
  theme_bw()
 
nc |>
  filter(SIDS_deaths_per_birth<0.0075) |>
   ggplot(aes(x=nonwhite_births, y=SIDS_deaths_per_birth)) +
  geom_point() +

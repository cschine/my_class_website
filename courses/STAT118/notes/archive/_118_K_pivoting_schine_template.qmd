...
# same thing with column numbers
long_portal <- wide_portal |>
...
# fille missing value with missing value numeric code
example |>
  pivot_wider(names_from=month,
              values_from=avg_weight,
              values_fill=-999)
...
portal_rodent |>
  group_by(plot, year) |>
  summarize(spp_num=n_distinct(species)) |>
  pivot_wider(names_from=year, values_from=spp_num)
